"""Review explicit immutable September 24 models; no fitting or current pointers.
...
BASELINE = ROOT / "ml/nightly-gameplan-runs/20260923T054513.490247Z"
NATIVE = ROOT / "ml/overnight-runs/20260924T040733.615371Z"
GROUPS = ("1h", "4h", "1d", "1w")
...
def group_review(run, reports, forecasts, group, previous):
    ...
    comparison = {"baseline_run": str(BASELINE), "previous_gate": previous["promotion_gate"],
                  ...
                  "different_feature_contract_and_cohorts": True,
                  "comparability_note": "Previous publication used the legacy prior-session feature contract; this is a factual cross-publication score comparison, not a controlled same-cohort accuracy experiment."}
...
def main():
    ...
    while ancestor is not None:
        ...
        require(pd.Timestamp(record["deadline_at"]) == pd.Timestamp("2026-09-24T11:00:00Z")
                and pd.Timestamp(record.get("effective_deadline_at", record["deadline_at"])) == pd.Timestamp("2026-09-24T11:00:00Z")
    ...
    require(publication.receipt["action_date"] == "2026-09-24", "Wrong action date")
    require(pd.Timestamp(publication.receipt["published_at"]) < pd.Timestamp("2026-09-24T11:00:00Z"),
    ...
    require(baseline.receipt["action_date"] == "2026-09-23", "Baseline date changed")
    ...
    review = {"reviewed_at": datetime.now(timezone.utc).isoformat(), "run": str(run), "action_date": "2026-09-24",
    ...
    metric_summary = {"reviewed_at": review["reviewed_at"], "run": str(run), "native_run": str(native_run),
        "action_date": "2026-09-24", "status": review["status"], "full_native_completion_verified": False,
    ...
    lines = ["# September 24 Gameplan model review", "", f"Reviewed {review['reviewed_at']}; immutable publication `{run}`.", "",
