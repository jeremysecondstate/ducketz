"""Offline audit of the September 25 source / September 28 action overnight run.
...
Based on the reconciled September 15 operator verifier, retaining its source,
model, forecast, cash/share, evaluation and actuals checks, including the exact
union of successful OPRA Historical and Live replay scopes. A failed ancestor
...
New planning projections use the September 16 signal-driven holding policy:
bullish accumulation and same-horizon bearish sales, with no expiry sales.
"""
...
parser.add_argument("--expected-original-run", default="20260926T040723.834511Z")
parser.add_argument("--expected-source-date", default="2026-09-25")
parser.add_argument("--expected-action-date", default="2026-09-28")
parser.add_argument("--expected-deadline", default="2026-09-28T11:00:00Z")
...
# The supervisor runs the full audit only after native completion. Refuse an
# early invocation before importing archive/model readers or touching pointers.
# This reads two small run-local JSON files and does not act on a running owner.
preflight_run = args.overnight_run.resolve()
preflight_paths = {name: preflight_run / name for name in ("stage-report.json", "receipt.json")}
preflight = {}
for name, path in preflight_paths.items():
    if path.is_file():
        preflight[name] = json.loads(path.read_text(encoding="utf-8"))
if any(preflight.get(name, {}).get("status") != "COMPLETE" for name in preflight_paths):
    print(json.dumps({
        "status": "NOT_READY_FOR_FINAL_VERIFICATION",
        "verified_at": datetime.now(timezone.utc).isoformat(),
        "overnight_run": str(preflight_run),
        "verification_is_read_only": True,
        "native_statuses": {name: preflight.get(name, {}).get("status", "MISSING") for name in preflight_paths},
        "archive_checks_started": False,
        "next_step": "Wait for native COMPLETE report and receipt, then verify the terminal attempt.",
...
from ml.gameplan_cash_ledger import (
    project_direction_trades, UnavailablePlanningPricePath, SIGNAL_DRIVEN_HOLDING_POLICY,
)
...
summary = {"verified_at": pd.Timestamp.now(tz="UTC").isoformat(),
           ...
           "verifier": {"path": str(Path(__file__).resolve()),
                        ...
                        "expected_original_run": args.expected_original_run,
                        "expected_source_date": args.expected_source_date,
                        "expected_action_date": args.expected_action_date,
...
def section(name, function):
    print("VERIFY_START " + name, file=sys.stderr, flush=True)
    try:
...
def check_overnight():
    ...
    while current:
        ...
        require(receipt.get("completed_at") == report.get("completed_at")
                and receipt.get("failed_stage") == report.get("failed_stage"),
                "Overnight receipt completion/failure identity differs")
        require(report["status"] in ({"COMPLETE"} if current == overnight else {"FAILED", "CANCELLED"}),
                "Native ancestry reuses an ineligible terminal attempt")
        recorded_stages = report.get("stages", [])
        attempted = [row["stage"] for row in recorded_stages]
        require(attempted == report["stage_order"][:len(attempted)] and len(attempted) == len(set(attempted)),
                "Native attempted stages are not the unique recorded order prefix")
        if report["status"] == "COMPLETE":
            require(attempted == report["stage_order"]
                    and all(row["status"] == "COMPLETE" for row in recorded_stages)
                    and report.get("current_stage") is None,
                    "COMPLETE native report does not contain every recorded successful stage")
        else:
            require(report.get("failed_stage") in report["stage_order"]
                    and all(row["status"] == "COMPLETE" for row in recorded_stages[:-1]),
                    "Failed native ancestor has an invalid completed-stage prefix")
        logged = receipt.get("logs", {})
        require(set(logged) == {path.name for path in current.glob("*.log")},
                "Native receipt log inventory omits or adds a run-local log")
        last_finished = pd.Timestamp(report["run_timestamp"])
        for stage in recorded_stages:
            start, finish = pd.Timestamp(stage["started_at"]), pd.Timestamp(stage["finished_at"])
            require(last_finished <= start <= finish <= pd.Timestamp(report["completed_at"]),
                    "Native stage clocks overlap or escape the attempt")
            last_finished = finish
            require(stage.get("log_path") == stage["stage"] + ".log",
                    "Native stage log path differs from its stage identity")
            if stage["status"] == "COMPLETE":
                require(stage["log_path"] in logged and stage.get("error") is None,
                        "Successful native stage lacks its bound log or records an error")
        ...
        require(pd.Timestamp(report.get("effective_deadline_at", deadline)) == pd.Timestamp(deadline)
                ...
                "This ordinary preparation cannot reuse the September 14 deadline exception")
        if pinned is None:
        ...
        require(report.get("stock_price_source") == "xnas-itch-archive-v1"
                ...
                and report.get("archive_history") is True
                and report.get("probability_target_contract") == "raw-price-direction-v1"
                and report.get("gameplan_variant") == "YG"
                and report.get("preparation_scope") == "STOCK_ONLY"
                and report.get("target_contract_version") == "independent-stock-targets-v1",
        ...
        for stage in report.get("stages", []):
            if stage["status"] == "COMPLETE":
                ...
                require(stage["stage"] not in completed,
                        "Resume reran an already successful original stage: " + stage["stage"])
                completed[stage["stage"]] = {**stage, "overnight_run": str(current)}
    ...
    inherited, previous = [], None
    for record in reversed(attempt_reports):
        require(record.get("completed_stages_from_previous_attempt", []) == inherited,
                "Resume inherited successful-stage evidence differs from its exact ancestry")
        if previous is not None:
            require(record["stage_order"][0] == previous["failed_stage"]
                    and pd.Timestamp(record["run_timestamp"]) >= pd.Timestamp(previous["completed_at"]),
                    "Resume did not start at its failed stage after the parent completed")
        inherited.extend(row["stage"] for row in record["stages"] if row["status"] == "COMPLETE")
        previous = record
    ...
    return {"attempt_chain": chain, "completed_stages": list(completed), "deadline_at": deadline,
            ...
            "log_checksums_verified": True, "complete_log_inventory_verified": True,
            "successful_stages_not_repeated": True, "exact_resume_stage_inheritance_verified": True}
...
def check_gameplan():
    ...
    for field in ("strategy_name", "strategy_display_name", "candidate_key", "candidate_rank", "legs_json",
                  "entry_net_credit", "entry_net_debit", "capital_required", "max_profit", "max_loss",
                  "decision_score", "score_basis", "pricing_status", "pricing_source",
                  "maximum_quote_staleness_seconds", "strategy_source_run"):
    ...
    aligned_intents = intents.set_index(["symbol", "route"]).sort_index()
    aligned_forecasts = forecasts.set_index(["symbol", "route"]).sort_index()
    for field in ("model_group", "forecast_anchor_local", "action_anchor_local", "direction", "execution_authority"):
        pd.testing.assert_series_equal(aligned_intents[field], aligned_forecasts[field],
                                       check_names=False, check_dtype=False, check_exact=True)
    for field in ("target_window_start", "target_window_end"):
        pd.testing.assert_series_equal(pd.to_datetime(aligned_intents[field], utc=True),
                                       aligned_forecasts[field], check_names=False, check_dtype=False, check_exact=True)
    pd.testing.assert_series_equal(aligned_intents.direction_probability_up,
                                   aligned_forecasts.calibrated_probability,
                                   check_names=False, check_dtype=False, check_exact=True)
    require(intents.broker_orders_enabled.eq(False).all() and intents.same_legs_revalidation_only.eq(True).all(),
            "Stock-only placeholders changed disabled option order authority")
    ...
    require(all(item.get("preparation_scope") == "STOCK_ONLY"
                and item.get("probability_target_contract") == "raw-price-direction-v1"
                and item.get("gameplan_variant") == "YG" for item in (cfg, pub.receipt, plan)),
            "Publication configuration, receipt or plan changed stock-only YG identity")
...
def check_enrichment():
    ...
    from ml.stock_trader.independent_training import ARCHIVE_MARKET_ADMISSION_POLICY
    ...
    payload = read(run / "model.json")
    admission_policy = payload.get("market_feature_admission_policy")
    require(admission_policy == ARCHIVE_MARKET_ADMISSION_POLICY
            and manifest["configuration"].get("market_feature_admission_policy") == admission_policy,
            "Archive enrichment model/manifest admission policy differs")
    for group, horizon in report["horizons"].items():
        source = pd.read_parquet(context["publication"].run_directory / f"training-cohort-{group}.parquet")
        candidates = source.loc[source.target_role.eq("EXECUTION") & source.execution_eligible.eq(True)]
        excluded = candidates.loc[candidates.source_selection_contract.eq("xnas-archive-prior-session-features-v1")
                                  & candidates[list(INDEPENDENT_MARKET_FEATURE_NAMES)].isna().all(axis=1)]
        ...
        key_records = json.loads(keys.to_json(orient="records", date_format="iso", double_precision=15))
        admission = horizon.get("market_feature_admission", {})
        expected = {"policy": admission_policy, "input_execution_rows": len(candidates),
            "admitted_rows": len(candidates) - len(excluded), "excluded_missing_market_rows": len(excluded),
            "excluded_by_symbol": {str(key): int(value) for key, value in excluded.symbol.value_counts().sort_index().items()},
            "excluded_by_feature": {name: int(excluded[name].isna().sum()) for name in INDEPENDENT_MARKET_FEATURE_NAMES},
            "excluded_rows_sha256": canonical_sha256(key_records)}
        require(all(admission.get(key) == value for key, value in expected.items())
                and payload["horizons"][group].get("market_feature_admission") == admission
                and horizon["admitted_rows"] == expected["admitted_rows"]
                and horizon["excluded_context_rows"] == len(source) - len(candidates),
                "Enrichment optional-input exclusions differ from immutable source cohort: " + group)
    horizons = {group: {key: value.get(key) for key in ("status", "reason", "admitted_rows", "fitted_scope_count",
                    ...
                    "excluded_context_rows", "market_feature_admission")}
    ...
    return {"run": str(run), "source_gameplan_run": report["source_gameplan_run"], "horizons": horizons,
            "market_feature_admission_policy": admission_policy,
            "optional_input_exclusion_evidence_recomputed": True}
...
def check_archive_history():
    from verify_archive_history import verify_archive_history
    return verify_archive_history(root=root, context=context, read=read, require=require,
                                  input_matches=input_matches, local_prices=local_prices,
                                  completed_history=summary["checks"]["stock_history"])
...
def check_signal_driven_holdings(rows, snapshot, ledger):
    """Independently roll horizon ownership forward without expiry transitions."""
    from decimal import Decimal
    from ml.stock_direction_policy import stock_direction
    zero = Decimal(0)
    symbols = context["symbols"]
    buckets = {(symbol, horizon): zero for symbol in symbols for horizon in ("1h", "4h", "1d", "1w")}
    reserved = buckets.copy()
    free = {symbol: Decimal(str(snapshot["held_shares"][symbol])) for symbol in symbols}
    for allocation in snapshot["ownership"].get("active_allocations", []):
        symbol = allocation["symbol"]
        if symbol not in free:
            continue
        key = symbol, allocation["horizon"]
        owned = Decimal(str(allocation["owned_shares"]))
        held_reserved = Decimal(str(allocation.get("reserved_sell_shares", 0)))
        require(key in buckets and zero <= held_reserved <= owned, "Invalid saved horizon ownership")
        buckets[key] += owned - held_reserved
        reserved[key] += held_reserved
        free[symbol] -= owned
    unallocated_reserved = {}
    for symbol in symbols:
        owned_reserved = sum((amount for key, amount in reserved.items() if key[0] == symbol), zero)
        unallocated_reserved[symbol] = Decimal(str(snapshot["pending_sell_shares"].get(symbol, 0))) - owned_reserved
        free[symbol] -= unallocated_reserved[symbol]
        require(free[symbol] >= zero and unallocated_reserved[symbol] >= zero,
                "Pending shares and initial ownership overdraw available holdings")
    by_id = rows.set_index("id").to_dict("index")
    seen = set()
    for event in ledger["events"]:
        identity = event["forecast_id"]
        require(identity in by_id and identity not in seen, "Projected instruction is missing or duplicated")
        seen.add(identity)
        row = by_id[identity]
        ...
        require(row["execution_eligible"] and key == (row["symbol"], row["model_group"])
                and pd.Timestamp(event["timestamp"]) == pd.Timestamp(row["target_window_start"])
                and quantity > zero and quantity == int(quantity),
                "Projected event changed its frozen instruction, clock or whole-share quantity")
        direction = stock_direction(row["calibrated_probability"])
        if event["action"] == "BUY":
            require(event["reason"] == "BULLISH_BUY" and direction == "BULLISH",
                    "Buy lacks a bullish instruction")
            buckets[key] += quantity
        else:
            require(event["action"] == "SELL" and event["reason"] == "BEARISH_SELL" and direction == "BEARISH",
                    "Signal-driven holdings contain an expiry or non-bearish sale")
            require(quantity <= free[key[0]] + buckets[key],
                    "Bearish sale takes reserved shares or another horizon's holdings")
            assigned = min(free[key[0]], quantity)
            free[key[0]] -= assigned
            buckets[key] -= quantity - assigned
    ending = {key: zero for key in buckets}
    ending_reserved = ending.copy()
    for lot in ledger["ending_allocations"]:
        key = lot["symbol"], lot["horizon"]
        require(key in ending, "Ending allocation belongs to an unknown horizon")
        ending[key] += Decimal(str(lot["quantity"]))
        ending_reserved[key] += Decimal(str(lot["reserved"]))
    require(ending == buckets and ending_reserved == reserved,
            "Accumulated horizon shares or protected reservations fail independent rollforward")
    for symbol in symbols:
        total = free[symbol] + unallocated_reserved[symbol] + sum(
            (buckets[key] + reserved[key] for key in buckets if key[0] == symbol), zero)
        require(same_number(total, ledger["ending_positions"][symbol]),
                "Ending symbol holdings do not reconcile to separate horizon ownership")
    return {"policy": SIGNAL_DRIVEN_HOLDING_POLICY, "scheduled_expiry_sales": 0,
            "unique_instruction_events": len(seen), "horizon_ownership_rollforward": "VERIFIED",
...
def check_trade_plan():
    ...
    require(report.get("planning_override") is None,
            "Current overnight planning must retain the original safe snapshot without an override")
    require(all(item.get("probability_target_contract") == "raw-price-direction-v1"
                and item.get("gameplan_variant") == "YG"
                for item in (report, receipt, manifest["configuration"])),
            "Trade-plan report/receipt/manifest changed the YG target identity")
    require(rows.probability_target_contract.eq("raw-price-direction-v1").all()
            and rows.gameplan_variant.eq("YG").all(),
            "Augmented trade rows changed the frozen YG target identity")
    ...
    if ledger.get("status") == "COMPLETE":
        ...
        require(ledger.get("holding_policy") == SIGNAL_DRIVEN_HOLDING_POLICY,
                "New trade projection did not select the signal-driven holding policy")
        require("Holdings have no scheduled expiry sale." in readable
                and "Forecast boundaries do not sell shares." in readable,
                "Readable review lost the signal-driven holding explanation")
    ...
    signal_holdings = check_signal_driven_holdings(rows, snapshot, ledger)
    ...
    return {**common, "run": str(run), "readable_gameplan": str(run / "Gameplan.md"), "rows": len(rows),
            ...
            "standalone_capacity_recomputed": True, "signal_driven_holdings": signal_holdings,
...
def check_actuals():
    ...
    boundary_diagnostics = {
        label: {field: frame[field].value_counts(dropna=False).to_dict()
                for field in frame.columns if field in columns}
        for label, frame, columns in (
            ("forecasts", frozen_results, {"actual_start_status", "actual_end_status",
                                          "actual_start_source_coverage", "actual_end_source_coverage"}),
            ("same_clock_prices", price_results, {"actual_status", "actual_source_coverage"}),
        )
    }
...
if args.only_fetch_log:
    ...
    selected_checks = (("overnight", check_overnight), ("gameplan", check_gameplan),
        ("opra_coverage", check_opra), ("stock_history", check_stock_history), ("archive_history", check_archive_history),
        ("fetch_scope_log", check_fetch_log),
