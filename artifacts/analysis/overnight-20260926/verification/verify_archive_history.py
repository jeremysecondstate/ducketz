"""Read-only archive audit extension for the September 28 full-run verifier.
...
def verify_archive_history(*, root, context, read, require, input_matches,
    ...
    for group, regenerated in groups.items():
        ...
        require(actual.target_price_source_contract.eq('xnas-itch-archive-v1').all()
                ...
                and actual[['target_start_gap_seconds', 'target_end_gap_seconds']].abs().le(300).all().all(),
...
def _verify_extension(root, run, *, read, require, symbols, file_checksum):
    ...
    require(len(evidence['completed_requests']) == len(completed) == len(requests)
            and set(completed) == {r['request_id'] for r in requests}, 'Extension incomplete or duplicated request set')
    details = []
    ...
    if requests:
        ...
        require(len(preflight['estimates']) == len(estimates) == len(requests)
                and set(estimates) == set(completed), 'Extension exact metadata request scope differs')
