"""Review explicit immutable September 28 models; no fitting or current pointers.
...
BASELINE = ROOT / "ml/nightly-gameplan-runs/20260925T060654.631426Z"
...
def group_review(run, reports, forecasts, group, previous):
    ...
    comparison = {"baseline_run": str(BASELINE), "previous_gate": previous["promotion_gate"],
                  ...
                  "comparability_note": "Previous publication is the frozen September 25 archive-history baseline; feature-contract identity is compared explicitly. This is a factual cross-publication score comparison, not a controlled same-cohort accuracy experiment."}
...
def enrichment_review(path, gameplan_run):
    ...
    from ml.stock_trader.independent_training import _quality_passes, MINIMUM_ASSESSMENT_CLUSTERS
    run = path.resolve()
    ...
    for group in GROUPS:
        ...
        if r["status"] == "FITTED":
            ...
            scores = first['horizon_assessment_scores']
            quality_checks = {
                'brier_below_baseline': scores['brier'] < scores['base_brier'],
            ...
            checks['strict_sizing_quality_arithmetic'] = all(quality_checks.values()) == _quality_passes(scores)
            quality_checks['assessment_clusters_at_least_minimum'] = r['partition_evidence']['assessment']['decision_clusters'] >= MINIMUM_ASSESSMENT_CLUSTERS
        groups[group] = {"fit_status": r["status"], "fitted_scopes": r.get("fitted_scope_count", 0),
            ...
            "admitted_rows": r.get("admitted_rows"), "excluded_context_rows": r.get("excluded_context_rows"),
            "market_feature_admission": r.get("market_feature_admission"),
            "sizing_quality_checks": quality_checks,
            "failed_sizing_quality_checks": [name for name, passed in quality_checks.items() if not passed],
...
def main():
    ...
    while ancestor is not None:
        ...
        require(pd.Timestamp(record["deadline_at"]) == pd.Timestamp("2026-09-28T11:00:00Z")
                and pd.Timestamp(record.get("effective_deadline_at", record["deadline_at"])) == pd.Timestamp("2026-09-28T11:00:00Z")
    ...
    require(publication.receipt["action_date"] == "2026-09-28", "Wrong action date")
    require(pd.Timestamp(publication.receipt["published_at"]) < pd.Timestamp("2026-09-28T11:00:00Z"),
    ...
    require(baseline.receipt["action_date"] == "2026-09-25", "Baseline date changed")
    ...
    review = {"reviewed_at": datetime.now(timezone.utc).isoformat(), "run": str(run), "action_date": "2026-09-28",
    ...
    metric_summary = {"reviewed_at": review["reviewed_at"], "run": str(run), "native_run": str(native_run),
        "action_date": "2026-09-28", "status": review["status"], "full_native_completion_verified": False,
    ...
    lines = ["# September 28 Gameplan model review", "", f"Reviewed {review['reviewed_at']}; immutable publication `{run}`.", "",
