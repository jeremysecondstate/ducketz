...
def main():
    ...
    review = {"reviewed_at": datetime.now(timezone.utc).isoformat(), "status": "PENDING",
        ...
        "implementation": implementation, "gameplan_run": str(run), "action_date": ACTION, "publication_stage": stages[0], "pinned_receipt_sha256": pin["receipt_sha256"],
    ...
    lines = ["# September 28 preliminary directional review", "", "Pinned publication: `"+str(run)+"`.", "",
    ...
    lines += ["", "Saved policy permits Brier baseline +0.005 and log-loss baseline +0.01; qualification within those tolerances does not establish baseline outperformance.", "", "Horizons strictly beating both training baselines: "+", ".join(g for g,v in review["strict_baseline_comparison"].items() if v)+". Other groups keep their actual saved qualification status and metric margins.", "",
        "Per-symbol/route fitted and assessment counts are retained exactly in the JSON, including thin TWST histories. These are observed saved counts, not a new qualification rule.", "", review["support_limitation"], "", review["repair_assessment"], "",
