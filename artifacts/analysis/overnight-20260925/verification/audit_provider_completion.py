"""Bounded read-only audit of the September 25 overnight provider stage.

Run only after native COMPLETE. Only the report/helper directory is writable. Reads the current native log,
...
EXPECTED_CYCLE_ID = '20260925T040822.974281Z-pid22132'
OPRA = ROOT / 'market-data/databento/opra/OPRA.PILLAR'
SOURCE, END, ACTION = '2026-09-24', '2026-09-25', '2026-09-25'
...
args.add_argument('--terminal-run', type=Path, default=RUN,
                  help='Completed terminal attempt, descending from the original scheduled run')
...
terminal = args.terminal_run.resolve()
if terminal.parent != (ROOT / 'ml/overnight-runs').resolve():
    raise ValueError('Terminal attempt is outside the native overnight root')
terminal_report_path, terminal_receipt_path = terminal/'stage-report.json', terminal/'receipt.json'
terminal_report = load(terminal_report_path) if terminal_report_path.is_file() else {}
terminal_receipt = load(terminal_receipt_path) if terminal_receipt_path.is_file() else {}
if terminal_report.get('status') != 'COMPLETE' or terminal_receipt.get('status') != 'COMPLETE':
    print(json.dumps({'status': 'NOT_READY_FOR_FINAL_VERIFICATION', 'native_run': str(terminal),
                      'native_report_status': terminal_report.get('status', 'MISSING'),
                      'native_receipt_status': terminal_receipt.get('status', 'MISSING'),
                      'archive_checks_started': False, 'provider_calls': 0}))
    raise SystemExit(2)
if (terminal_receipt.get('stage_report_checksum_sha256') != digest_file(terminal_report_path)
        or terminal_receipt.get('stage_report_size') != terminal_report_path.stat().st_size):
    raise ValueError('Terminal receipt does not bind the completed native report')
ancestor, seen = terminal, set()
while ancestor != RUN.resolve():
    if ancestor.parent != (ROOT/'ml/overnight-runs').resolve() or ancestor in seen:
        raise ValueError('Invalid terminal native ancestry')
    seen.add(ancestor)
    record = load(ancestor/'stage-report.json')
    if not record.get('resumed_from'):
        raise ValueError('Terminal attempt does not descend from the expected original native run')
    ancestor = Path(record['resumed_from']).resolve()
...
result = {
    ...
    'production_symbols': SYMBOLS, 'native_run': str(RUN), 'terminal_native_run': str(terminal), 'base_cycle': cycle, 'base_complete_receipt_matches': cycle == complete,
...
md = [
    ...
    'Full evidence: [provider-completion.json](C:/dev/ducketz/artifacts/analysis/overnight-20260925/verification/provider-completion.json).', '',
]
if issues:
