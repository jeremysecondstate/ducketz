...
def compare_saved_numbers(actual, expected, label):
    """Compare the full saved report structure, tolerating inference roundoff only."""
    if isinstance(expected, dict):
        require(isinstance(actual, dict) and set(actual) == set(expected), label + ': keys differ')
        for key in expected:
            compare_saved_numbers(actual[key], expected[key], label + '/' + str(key))
    elif isinstance(expected, (list, tuple)):
        require(isinstance(actual, (list, tuple)) and len(actual) == len(expected), label + ': length differs')
        for index, (left, right) in enumerate(zip(actual, expected)):
            compare_saved_numbers(left, right, label + '/' + str(index))
    elif isinstance(expected, (int, float)) and not isinstance(expected, bool):
        require(not isinstance(actual, bool) and math.isfinite(float(actual))
                and math.isfinite(float(expected))
                and math.isclose(float(actual), float(expected), rel_tol=0, abs_tol=1e-12),
                label + ': value differs')
    else:
...
def main():
    ...
    require(output.is_relative_to((REPOSITORY/'artifacts/analysis/overnight-20260925/verification').resolve()),
    ...
    import numpy as np
    import pandas as pd
    from ml.nightly_gameplan import (read_gameplan_run, _chronological_partitions, _model_frame,
        _proper_scores, _verify_probability_cohort, _calibration_signal_diagnostics,
        _symbol_target_support, GAMEPLAN_VERSION)
    ...
    try:
        ...
        while current:
            ...
            require(target_matches(r) and r.get('archive_history') is True, 'Native ancestry changed YG/archive identity')
        ...
        require(cfg.get('archive_history') is True and cfg.get('source_selection_contract') == 'xnas-archive-prior-session-features-v1', 'Publication archive feature policy differs')
    ...
    if run is not None:
        for group in GROUPS:
            try:
                ...
                require((run/model_name).stat().st_size == r['model_file']['size']
                        and sha(run/model_name) == r['model_file']['checksum_sha256'],
                        'Every fitted artifact must agree with its report hash and size')
                payload = joblib.load(run/model_name)
                require(target_matches(r) and target_matches(payload), 'Report/model target differs')
                require(r.get('schema_version') == payload.get('schema_version') == GAMEPLAN_VERSION
                        and r.get('group') == payload.get('group') == group
                        and r.get('source_selection_contract') == payload.get('source_selection_contract') == cfg['source_selection_contract']
                        and tuple(payload.get('feature_columns', ())) == tuple(r['features']['admitted'])
                        and len(payload.get('feature_columns', ())) == r['features']['admitted_count'] > 0
                        and tuple(payload.get('categorical_columns', ())) == ('symbol', 'route')
                        and getattr(payload.get('calibrator'), 'method', None) == r['calibration_method']
                        and callable(getattr(payload.get('estimator'), 'predict_proba', None))
                        and pd.Timestamp(payload['trained_at']) <= pd.Timestamp(publication.receipt['published_at']),
                        'Fitted model schema, features, source, calibrator, or fit timestamp differs')
                for field in ('selected_family', 'selected_neural_weight', 'selected_logistic_regularization_c',
                              'selection_metrics', 'calibration_selection', 'target_calendar_feature_contract'):
                    require(payload.get(field) == r.get(field), 'Fitted model report differs: ' + field)
                current_rows = forecasts.loc[forecasts.model_group.eq(group)]
                require(current_rows.model_artifact.eq(model_name).all()
                        and current_rows.model_family.eq(r['selected_family']).all()
                        and current_rows.calibration_method.eq(r['calibration_method']).all()
                        and current_rows.calibration_status.eq(r['calibration_diagnostics']['status']).all()
                        and np.isfinite(current_rows.raw_probability).all()
                        and current_rows.raw_probability.between(0, 1).all(),
                        'Forecast rows disagree with fitted model identity or raw probability domain')
                ...
                for name, values in (('assessment', scores), ('assessment_raw_scores', raw_scores),
                                     ('training_base_rate_assessment', baseline)):
                    compare_saved_numbers(values, r[name], group + '/' + name)
                ...
                compare_saved_numbers(_proper_scores(cal.target.to_numpy(), cal_raw),
                                      r['calibration_raw_scores'], group + '/calibration_raw_scores')
                diagnostics = _calibration_signal_diagnostics(payload['calibrator'], cal_raw, cal.target.to_numpy(), prob)
                compare_saved_numbers(diagnostics, r['calibration_diagnostics'], group + '/calibration_diagnostics')
                assessed = assessment.copy()
                assessed['probability'] = prob
                route_scores = {str(route): _proper_scores(rows.target.to_numpy(), rows.probability.to_numpy())
                                for route, rows in assessed.groupby('route', sort=True)}
                support, symbol_scores = _symbol_target_support(cohort, fitted=fit, assessment=assessed, symbols=symbols)
                compare_saved_numbers(route_scores, r['assessment_by_route'], group + '/assessment_by_route')
                compare_saved_numbers(symbol_scores, r['assessment_by_symbol'], group + '/assessment_by_symbol')
                compare_saved_numbers(support, r['target_support_by_symbol'], group + '/target_support_by_symbol')
                for metric in ('brier_score', 'log_loss'):
                    compare_saved_numbers(scores[metric] - raw_scores[metric],
                                          r['calibration_assessment_change'][metric], group + '/calibration_change/' + metric)
                ...
                if deployment:
                    ...
                    checks['retained_champion_same_feature_contract'] = champion.manifest['configuration'].get('source_selection_contract') == cfg['source_selection_contract']
                shrinkage = None
                if group == '1w':
                ...
                detail = {'status': 'VERIFIED' if not failed else 'FAILED', 'checks': {k: bool(v) for k,v in checks.items()},
                    ...
                    'score_max_abs_error': max(errors), 'assessment_route_scores_reproduced': len(route_scores),
                    'assessment_symbol_scores_reproduced': len(symbol_scores),
                    'calibration_diagnostics_and_all_support_counts_reproduced': True, 'assessment': scores, 'raw_assessment': raw_scores, 'baseline': baseline,
    ...
    if args.publication_only:
        ...
        result['native_terminal_receipt_and_saved_models_verified'] = not bool(result['errors'])
        # This supplement does not validate all provider, planning, actuals and
        # source outputs; the separate full verifier owns that conclusion.
        result['full_native_completion_verified'] = False
    result['completion_scope_note'] = 'Full overnight completion requires the separate verify_completed_run.py output; this supplement covers the terminal receipt and saved directional models.'
