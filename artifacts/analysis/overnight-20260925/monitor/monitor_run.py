...
run = Path('C:/DATASTORE/ml/overnight-runs/20260925T040822.112032Z')
...
for path in sorted(run.glob('*.log')):
    ...
    if lines:
        issues = list(dict.fromkeys(line for line in lines if re.search(r'error|exception|traceback|warning|fail|unavailable|denied|quarantined', line, re.I)))
        result['new_logs'][path.name] = {'new_lines': len(lines), 'issues': issues, 'tail': lines[-3:]}
...
result['new_opra_preflights'] = []
for path in preflight_root.glob('*/*/*/preflight.json'):
    if path.stat().st_mtime < 1790310780:
        continue
    key = 'preflight:' + str(path)
    if state.get(key) == path.stat().st_mtime_ns:
        continue
    payload = json.loads(path.read_text(encoding='utf-8'))
    result['new_opra_preflights'].append({'path': str(path), 'cost': payload.get('estimated_cost_usd'), 'cost_estimates_complete': payload.get('cost_estimates_complete'), 'capacity_pass': payload.get('capacity_pass'), 'scope': payload.get('scope')})
    state[key] = path.stat().st_mtime_ns
...
compact['new_opra_preflights'] = result['new_opra_preflights']
