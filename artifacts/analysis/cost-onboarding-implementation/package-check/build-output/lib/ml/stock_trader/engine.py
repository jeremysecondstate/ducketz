...
def build_trade_decisions(
    ...
    *,
    ...
    """Create one auditable decision per configured symbol from a shared snapshot.
    ...
    for candidate in buys:
        ...
        if candidate.quantity <= 0:
            ...
            candidate.eligibility_reason = (
                "Joint portfolio cash/gross allocation left no whole-share quantity."
            )
            ...
            candidate.eligibility_reason = (
                "Joint portfolio sizing reduced the action below minimum notional."
            )
