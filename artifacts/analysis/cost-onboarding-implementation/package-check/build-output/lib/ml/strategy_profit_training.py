...
from datafetching.databento_opra_history import canonical_root, verify_partition
...
def _opra_partition_path(
    ...
    historical = segments / "full-day" / "normalized.parquet"
    if historical.is_file():
        return historical
    replay = tuple(sorted(path for path in segments.glob("live-session*") if path.is_dir()))
    if not replay:
        return historical
    if len(replay) != 1:
        raise RuntimeError(f"Ambiguous OPRA live-session partitions: {schema}/{symbol}/{session}")
    directory = replay[0]
    # Immutable partitions are repeatedly resolved while modeling outcomes. A
    # change to any bound evidence file requires fresh checksum verification.
    try:
        signature = tuple(
            (name, (directory / name).stat().st_size, (directory / name).stat().st_mtime_ns)
            for name in ("manifest.json", "receipt.json", "provider.dbn", "normalized.parquet")
        )
    except OSError as exc:
        raise RuntimeError(f"Incomplete OPRA live-session evidence: {directory}") from exc
    return _verified_live_session_path(
        Path(root).resolve(), directory.resolve(), schema, symbol, str(session), signature
    )


@lru_cache(maxsize=256)
def _verified_live_session_path(
    root: Path,
    directory: Path,
    schema: str,
    symbol: str,
    session: str,
    evidence_signature: tuple[tuple[str, int, int], ...],
) -> Path:
    del evidence_signature  # Used by the cache to invalidate changed evidence.
    ...
    if (
        ...
        or manifest.get("raw", {}).get("path") != "provider.dbn"
        or manifest.get("normalized", {}).get("path") != "normalized.parquet"
    ):
        raise RuntimeError(f"OPRA live-session partition identity is invalid: {directory}")
    try:
        label = pd.Timestamp(session)
        calendar = ExchangeSessionCalendar(
            "XNYS", start=label - pd.Timedelta(days=7), end=label + pd.Timedelta(days=7)
        )
        opened = calendar.session_open(label)
        closed = calendar.session_close(label)
        start, end = _utc(manifest["partition_start"]), _utc(manifest["partition_end"])
        request_start, request_end = _utc(request["start"]), _utc(request["end"])
        covered = (
            start == request_start
            and end == request_end
            and start <= opened
        ...
        raise RuntimeError(f"OPRA live-session coverage is invalid: {directory}") from exc
    if not covered:
        raise RuntimeError(f"OPRA live-session partition does not cover the complete session: {directory}")
    return directory / "normalized.parquet"
