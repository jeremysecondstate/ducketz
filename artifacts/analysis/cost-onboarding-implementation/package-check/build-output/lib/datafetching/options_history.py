...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--live-replay-fallback", action="store_true",
                        help="Complete a missing session using verified finite Live replay when Historical data is delayed.")
    parser.add_argument("--required-session", default=None,
                        help="Exact completed XNYS session (YYYY-MM-DD); required with --live-replay-fallback.")
    parser.add_argument(
    ...
    if args.live_replay_fallback and (not args.incremental_only or not args.required_session):
        parser.error("--live-replay-fallback requires --incremental-only and --required-session")
    if args.required_session and not args.live_replay_fallback:
        parser.error("--required-session requires --live-replay-fallback")
    try:
    ...
    summary = synchronize_option_history(
        ...
        live_replay_fallback=args.live_replay_fallback,
    ...
    print(
        ...
        f"live_replay_completed_scopes={summary.live_replay_completed_scopes}; "
        f"live_replay_bytes={summary.live_replay_bytes}; "
