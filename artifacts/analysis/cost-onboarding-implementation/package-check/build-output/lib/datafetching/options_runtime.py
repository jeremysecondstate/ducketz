...
class OptionHistorySyncSummary:
    ...
    live_replay_completed_scopes: int = 0
    live_replay_bytes: int = 0
...
def synchronize_option_history(
    ...
    *,
    ...
    live_replay_fallback: bool = False,
    ...
    if live_replay_fallback:
        from datafetching.opra_replay_fallback import replay_session_bounds
        if bootstrap_missing or required_session is None:
            raise ValueError("Live fallback requires incremental-only and an exact required session")
        for schema in clean_schemas:
            replay_session_bounds(required_session, schema)
    requested = completed = blocked = failed = needs_bootstrap = 0
    live_completed = live_bytes = 0
    integrity_failed_scopes: set[tuple[str, str]] = set()
    capacity_blocked_scopes: set[tuple[str, str]] = set()
    budget_deferred_scopes: set[tuple[str, str]] = set()
    bootstrap_missing_scopes: set[tuple[str, str]] = set()
        ...
        with exclusive_runtime_lock(
                ...
                for symbol in clean_symbols:
                    ...
                    if marker is None:
                        if not bootstrap_missing:
                            needs_bootstrap += 1
                            bootstrap_missing_scopes.add((symbol, schema))
                            if reporter is not None:
                                reporter(
                        ...
                        # A completed replay may be ahead of Historical metadata.
                        # Never regress its cursor or repeatedly fetch unchanged
                        # overlaps while the Historical archive is unavailable.
                        if completed_end > pd.Timestamp(entitled_end) or (
                            live_replay_fallback and completed_end == pd.Timestamp(entitled_end)
                        ):
                            if (not live_replay_fallback or completed_end >=
                                pd.Timestamp(required_session) + pd.Timedelta(days=1)):
                                completed += 1
                            continue
                        start = max(
            ...
            for plan in scope_plans:
                ...
                if guarded:
                    try:
                        ...
                        failed += 1
                        if _opra_history_integrity_failure(str(exc)):
                            integrity_failed_scopes.add((symbol, schema))
                        if reporter is not None:
                            reporter(
                    ...
                    if exceeds_bytes or exceeds_cost:
                        deferred += 1
                        budget_deferred_scopes.add((symbol, schema))
                        if reporter is not None:
                            reporter(
            ...
            for plan in selected_plans:
                ...
                try:
                    ...
                    blocked += 1
                    capacity_blocked_scopes.add((symbol, schema))
                    if reporter is not None:
                        reporter(
                    ...
                    if reporter is not None:
                        reporter(
                            ...
                            f"schema={schema}; {type(exc).__name__}: {exc}"
                        )
                    continue
                if result.errors or result.completed_rows < 1:
                    failed += 1
                    if any(_opra_history_integrity_failure(str(error)) for error in result.errors.values()):
                        integrity_failed_scopes.add((symbol, schema))
                    if reporter is not None:
                        reporter(
                            f"OPRA symbol/schema history incomplete: symbol={symbol}; "
            ...
            if live_replay_fallback and not preflight_only:
                from datafetching.opra_replay_fallback import complete_session_from_replay
                live_completed, live_failed, live_bytes = complete_session_from_replay(
                    store.root_dir, api_key=api_key, symbols=clean_symbols,
                    schemas=clean_schemas, session=str(required_session),
                    entitlement=entitlement,
                    max_bytes=(max_estimated_download_bytes or 2_000_000_000) - selected_estimated_bytes,
                    reporter=reporter,
                )
                # Historical and Live are two delivery attempts for the same
                # scope. Reconcile their final source evidence once; a provider
                # failure is resolved only when that exact scope is now current.
                required_end = pd.Timestamp(required_session) + pd.Timedelta(days=1)
                final_scopes: dict[tuple[str, str], bool] = {}
                for schema in clean_schemas:
                    for symbol in clean_symbols:
                        identity = (symbol, schema)
                        marker = _read_opra_symbol_history_cursor(
                            store.root_dir, symbol=symbol, schema=schema,
                        )
                        final_scopes[identity] = (
                            identity not in integrity_failed_scopes
                            and marker is not None
                            and pd.Timestamp(str(marker["completed_through"])) >= required_end
                        )
                completed = sum(final_scopes.values())
                unresolved = {identity for identity, current in final_scopes.items() if not current}
                failed = len(unresolved)
                blocked = len(unresolved.intersection(capacity_blocked_scopes))
                deferred = len(unresolved.intersection(budget_deferred_scopes))
                needs_bootstrap = len(unresolved.intersection(bootstrap_missing_scopes))
                health_refresh_pending = health_refresh_pending or live_completed > 0
    ...
    return OptionHistorySyncSummary(
        ...
        live_replay_completed_scopes=live_completed,
        live_replay_bytes=live_bytes,
    )


def _opra_history_integrity_failure(message: str) -> bool:
    """A new session's replay cannot repair corrupted Historical evidence."""
    text = message.lower()
    return any(token in text for token in (
        "checksum", "manifest", "receipt", "duplicate", "normalization",
        "normalized", "metadata does not match", "identity", "escapes",
    ))
    ...
    if "replay_coverage" in payload:
        ...
        try:
            verify_replay_cursor_coverage(
                datastore_root, symbol=symbol, schema=schema, cursor=payload,
            )
...
def _write_opra_symbol_history_cursor(
    ...
    *,
    ...
    replay_coverage: dict[str, object] | None = None,
) -> Path:
    ...
    if replay_coverage is not None:
        payload["replay_coverage"] = dict(replay_coverage)
