...
RUN = ROOT / 'ml/overnight-runs/20260923T040814.422433Z'
LOOP_B = ROOT / 'ml/runs/20260923T051825.677885Z'
PRIOR = Path('C:/dev/ducketz/artifacts/analysis/overnight-20260922/pricing-gate-review.json')
issues = []
compact_bytes = {}
def load(path):
...
check(base['generation'] == '20260923T040815.264882Z-pid33836' and base['status'] == 'COMPLETE', 'Base input cutoff is not current native cycle')
...
if current_manifest and current_publication:
    ...
    check(gate['model_admission_by_horizon'] == prior['native_model_admission_by_horizon'], 'Model admission differs from prior verified policy')
...
result = {'audited_at':datetime.now(timezone.utc).isoformat(),
    ...
    'model_training_completion_verified':False, 'log_path':str(log_path),
    ...
    'conclusion':'Same sparse stale August 20 Pricing authority and 99-route optional exclusion. Keep native gates and exact non-Pricing fallback; no new source defect or justified retry identified.'}
(OUT / 'pricing-gate-review.json').write_text(json.dumps(result,indent=2)+'\n', encoding='utf-8')
md = ['# September 23 Pricing gate review','',f"Reviewed {result['audited_at']}. **{result['status']}**.",'',
    ...
    f"Current causal cutoff: `{base['finished_at']}`. Current manifest/publication available: **{current_manifest}/{current_publication}**. Current saved route fractions: **{result['current_route_fraction_review']}**. No model or overnight completion claim is made.",'']
if group_summary:
...
md += ['',f"Discrepancies: **{len(issues)}**. Preserve native gates and reduced feature contracts; no fresh source defect or justified focused repair was found. OPRA completion does not create a Pricing publication. CME raw-contract limitations remain separate.",'',
       '[Exact evidence](C:/dev/ducketz/artifacts/analysis/overnight-20260923/pricing-gate-review.json)','']
