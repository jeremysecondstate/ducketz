"""Bounded native reconciliation of one verified, unfilled cancelled TWST order.
...
TOKEN = "858e2b00-276f-485e-946a-6e63b799ede3"
...
EXPECTED = {
    "a7083119fa00a0e82844e93f0d5958e997ff4ad76c34f27927273cb89d2d5055":
        ("TWST", "1h", "SELL", 38, "1008025436225", Decimal("166.31")),
}
FINAL_RUN = ROOT / "ml/stock-trader-decision-runs/20260923T000001.790133Z"
TERMINAL_ARTIFACT_SHA256 = {'state/independent-stock-trader/session-status.json': '88b99f0c50cff53a2a62d9accfaddecbe37ff1830b9231fd1cac65ccb798909b', 'ml/stock-trader-decision-runs/20260923T000001.790133Z/receipt.json': '53a8e38f1e890ba7875a678ab38b27b2ced629852265c15bf448f53c8e1013f9', 'ml/stock-trader-decision-runs/20260923T000001.790133Z/manifest.json': '36e167c525a92f2daa6c837065dc0cc1f59c22f15c8537ae1a99f0a39aa7472c', 'ml/stock-trader-decision-runs/20260923T000001.790133Z/decisions.json': '832e6df706d8fdcbe88c2b2a26b6a885d7b957cefec7e835cdaf1162b85e4c8f'}
...
def guarded_artifacts():
    ...
    paths.extend(FINAL_RUN / name for name in ("receipt.json", "manifest.json", "decisions.json"))
...
def check_finished_session():
    status = read(STATUS)
    require(status.get("status") == "FINISHED_WITH_ERRORS", "TRADER_SESSION_TERMINAL_STATE_CHANGED")
    require(status.get("action_date") == "2026-09-22", "TRADER_SESSION_DATE_CHANGED")
    require(status.get("pid") == 23456, "TRADER_SESSION_ID_CHANGED")
    require(status.get("started_at") == "2026-09-22T15:07:59.988392+00:00", "TRADER_SESSION_START_CHANGED")
    ...
    require(status.get("calls") == 1114 and status.get("orders_submitted") == 49
            and status.get("failed_cycles") == 1 and status.get("consecutive_failures") == 1,
            "TERMINAL_SESSION_COUNTS_CHANGED")
    last = status.get("last_cycle", {})
    require(last.get("status") == "EXECUTION_WINDOW_CLOSED_AFTER_BROKER_CAPTURE"
            and last.get("error") == "OUTSIDE_US_EQUITY_ACTIONABLE_SESSION"
            and last.get("selected_orders") == 0 and last.get("submitted_orders") == 0
            and Path(last.get("run_directory", "")).resolve() == FINAL_RUN.resolve(),
            "TERMINAL_CLOSING_BOUNDARY_EVIDENCE_CHANGED")
    require({name: sha(ROOT / name) for name in TERMINAL_ARTIFACT_SHA256}
            == TERMINAL_ARTIFACT_SHA256, "TERMINAL_ARTIFACT_CHANGED")
    receipt = read(FINAL_RUN / "receipt.json")
    decisions = read(FINAL_RUN / "decisions.json")
    require(receipt.get("manifest_sha256") == sha(FINAL_RUN / "manifest.json")
            and receipt.get("decisions_sha256") == sha(FINAL_RUN / "decisions.json"),
            "TERMINAL_RECEIPT_HASH_MISMATCH")
    require(receipt.get("status") == "EXECUTION_WINDOW_CLOSED_AFTER_BROKER_CAPTURE"
            and receipt.get("decided_at") == "2026-09-23T00:00:01.790133+00:00"
            and receipt.get("orders_selected") == 0 and receipt.get("decision_ids") == []
            and decisions.get("decisions") == []
            and receipt.get("prediction_handoff", {}).get("inventory_reconciliation")
            == {"ready": True, "reasons": []}, "TERMINAL_RECEIPT_NOT_EXPECTED_BOUNDARY_RESULT")
    ...
    require(not psutil.pid_exists(23456), "PRIOR_TRADER_PID_PRESENT")
    return status
...
def main():
    ...
    parser.add_argument("--apply", action="store_true", required=True,
                        help="Apply only the guarded native terminal TWST order reconciliation")
    parser.parse_args()
