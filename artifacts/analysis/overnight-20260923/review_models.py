"""Review explicit immutable September 23 models; no fitting or current pointers.
...
BASELINE = ROOT / "ml/nightly-gameplan-runs/20260922T054940.677673Z"
...
def main():
    ...
    while ancestor is not None:
        ...
        require(pd.Timestamp(record["deadline_at"]) == pd.Timestamp("2026-09-23T11:00:00Z")
                and pd.Timestamp(record.get("effective_deadline_at", record["deadline_at"])) == pd.Timestamp("2026-09-23T11:00:00Z")
        ...
        require(record.get("probability_target_contract") == TARGET and record.get("gameplan_variant") == "YG"
                ...
                and record.get("stock_price_source") == "xnas-itch-archive-v1", "Native source/target mode changed")
    ...
    require(publication.receipt["action_date"] == "2026-09-23", "Wrong action date")
    require(pd.Timestamp(publication.receipt["published_at"]) < pd.Timestamp("2026-09-23T11:00:00Z"),
    ...
    require(baseline.receipt["action_date"] == "2026-09-22", "Baseline date changed")
    ...
    review = {"reviewed_at": datetime.now(timezone.utc).isoformat(), "run": str(run), "action_date": "2026-09-23",
    ...
    metric_summary = {"reviewed_at": review["reviewed_at"], "run": str(run), "native_run": str(native_run),
        "action_date": "2026-09-23", "status": review["status"], "full_native_completion_verified": False,
    ...
    lines = ["# September 23 Gameplan model review", "", f"Reviewed {review['reviewed_at']}; immutable publication `{run}`.", "",
