...
The two added sizing columns use the normal configured bootstrap horizon: 5 days of `ohlcv-1s`, 100 days of `ohlcv-1m`, 2,000 days of `ohlcv-1h`, one calendar month of every remaining cold-start schema, and dataset-specific L0 daily/definition windows of **13 calendar years for OPRA**, **5,000 days for CME**, and **8 calendar years for US Equities**. The one-month non-L0 window is a deliberately bounded bootstrap scope; it is **not** an assertion that only one month is included wherever the entitlement tables show a deeper L0 or L1 window.
...
| ----- | ----------------------- | ---------------- | ----------- | --------------------- | --------------- |
| L0    | OHLCV-1s / 1m / 1h / 1d | Yes              | This produces `N × 9 columns`: one row per instrument per non-empty 1-second, 1-minute, 1-hour, or 1-day trade aggregate. Columns are `ts_event`, `rtype`, `publisher_id`, `instrument_id`, `open`, `high`, `low`, `close`, and `volume`. | Historical bootstrap: 5 days `1s`, 100 days `1m`, 2,000 days `1h`, and 13 calendar years `1d`; only non-empty bars. | `0.1043 × OHLCV rows_M`; no fixed OPRA-parent cap. |
...
| ----- | ----------------------- | ----------------------:| ----------- | --------------------- | --------------- |
| L0    | OHLCV-1s / 1m / 1h / 1d | 13+ years              | This produces `N × 9 columns`: one row per instrument per non-empty 1-second, 1-minute, 1-hour, or 1-day trade aggregate. Columns are `ts_event`, `rtype`, `publisher_id`, `instrument_id`, `open`, `high`, `low`, `close`, and `volume`. | 5 days `1s`, 100 days `1m`, 2,000 days `1h`, and 13 calendar years `1d`; only non-empty bars. | `0.1043 × OHLCV rows_M`; no fixed OPRA-parent cap. |
...
| Level | Schema / data type      | Live on Standard | Explanation | What would be fetched | How much in GiB |
| ----- | ----------------------- | ---------------- | ----------- | --------------------- | --------------- |
| —     | Full market summary     | Yes              | This is a bundle rather than one normalized schema: consolidated US-equity summary records combine daily OHLCV, statistics, and definitions. In Parquet, treat it as separate outputs—daily `ohlcv-1d` (`N × 9`), `statistics` (`N × 14`), and `definition` (`N × 73`)—rather than one rectangular table. | Reuse the requested daily OHLCV, 8-calendar-year definitions, and one-month statistics fetches; do not request a duplicate summary copy. | `0` incremental if those component files are reused. |
| L0    | OHLCV-1s / 1m / 1h / 1d | Yes              | This produces `N × 9 columns`: one row per instrument per non-empty 1-second, 1-minute, 1-hour, or 1-day trade aggregate. Columns are `ts_event`, `rtype`, `publisher_id`, `instrument_id`, `open`, `high`, `low`, `close`, and `volume`. | Historical bootstrap: 5 days `1s`, 100 days `1m`, 2,000 days `1h`, and 8 calendar years `1d`; only non-empty bars. | `0.1043 × OHLCV rows_M`; ≤`0.018` for all four windows of one always-active ticker. |
...
| ----- | ----------------------- | ----------------------:| ----------- | --------------------- | --------------- |
| —     | Full market summary     | Yes                    | This is a bundle rather than one normalized schema: consolidated US-equity summary records combine daily OHLCV, statistics, and definitions. In Parquet, treat it as separate outputs—daily `ohlcv-1d` (`N × 9`), `statistics` (`N × 14`), and `definition` (`N × 73`)—rather than one rectangular table. | Reuse the requested daily OHLCV, 8-calendar-year definitions, and one-month statistics fetches; do not request a duplicate summary copy. | `0` incremental if those component files are reused. |
| L0    | OHLCV-1s / 1m / 1h / 1d | 8+ years               | This produces `N × 9 columns`: one row per instrument per non-empty 1-second, 1-minute, 1-hour, or 1-day trade aggregate. Columns are `ts_event`, `rtype`, `publisher_id`, `instrument_id`, `open`, `high`, `low`, `close`, and `volume`. | 5 days `1s`, 100 days `1m`, 2,000 days `1h`, and 8 calendar years `1d`; only non-empty bars. | `0.1043 × OHLCV rows_M`; ≤`0.018` for all four windows of one always-active ticker. |
