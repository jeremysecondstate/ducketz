...
| Strategy runtime | None | None | Direct | Options | authoritative calibrated profitable-outcome probability when fitted, or separate non-probabilistic Scenario Coverage; expected profit/return and rank | read-only Options Strategy UI with research-only gating | `ml/strategy_selection/model.py`, `ml/strategy_runtime.py`, `ml/strategy_publication.py`, `app/ui/options_strategy_data.py` | High for contract; Unknown empirical performance |
...
2. **Confirmed:** it selects exact causal OPRA-first provider-neutral option-chain evidence, using verified Schwab history only when OPRA is unavailable, and separately uses Schwab stock BBO where required. It then constructs policy-eligible candidates and attaches receipt-proven Pricing evidence before any fitted probability score. Ready target sidecars are canonicalized to `BSGP`; complete residual fallbacks are `BLACK_SCHOLES`; unavailable/ineligible coverage cannot masquerade as fitted pricing. `ml/strategy_selection/chain.py`, `ml/strategy_selection/runtime.py`, `ml/option_pricing/strategy_shadow.py`
