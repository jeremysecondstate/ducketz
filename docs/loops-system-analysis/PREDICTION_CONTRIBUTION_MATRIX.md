...
3. **Confirmed:** historical exact entry/exit receipts produce strictly-positive-net-profit labels. The isolated daily Strategy-profit owner builds conservative execution evidence, keeps chronological train/calibration/assessment separation, and promotes only receipt-gated 1d/1w models. `ml/strategy_profit_training_runtime.py`, `ml/strategy_profit_training.py`, `ml/strategy_selection/model.py`
4. **Confirmed:** Strategy consumes that verified authority when compatible. Eligible candidates receive calibrated profitable-outcome probability and bounded expected profit/return; otherwise only separately typed Scenario Coverage remains and probability fields are null. Rows are deterministically ranked, validated and atomically published. `ml/strategy_selection/model.py`, `ml/strategy_selection/runtime.py`, `ml/strategy_runtime.py`, `ml/strategy_publication.py`
