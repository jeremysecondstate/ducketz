...
After parsing the guardian result, require the top-level `schedule` object with schema `loops-overnight-accuracy-schedule-v2`. Treat `schedule.monitor_mode`, `schedule.lane`, and `schedule.overnight_stage` as the sole routing authority; do not infer a stage from the clock or catch up a missed stage. Also preserve `session_close`, `equity_actionable_close`, and `equity_actionable_day_complete`: the 13:00 PT official close seals options/core evidence, while ordinary PRE/REGULAR/POST stock evidence remains provisional until 17:00 PT. Missing, malformed, or contradictory schedule metadata is a scheduling incident: preserve the guardian evidence, do not guess, and do not begin accuracy work. `STANDARD_OPERATIONS` means there is no overnight stage. `OVERNIGHT_ACCURACY` means execute at most the one named stage after health is proven. Every stage has a 45-minute ceiling and must checkpoint or defer before the next wake; never overlap stages or start a later stage early.
...
1. `seal-core-options-session` (13:42 PT): seal official regular-session and options publications, target boundaries, model generations, and fingerprints. Explicitly keep the still-open POST stock lane provisional; reject any artifact that claims the complete stock day before `equity_actionable_close`.
2. `audit-input-quality` (14:42): score expected-versus-received coverage, gaps, duplicates, ordering, timestamp/session alignment, schema/unit stability, null and missing-reason coverage, causal availability clocks, lineage, latency, and freshness against comparable trailing 5- and 20-session baselines when available. Segment stock evidence as PRE/REGULAR/POST and do not score the open POST tail as missing.
...
4. `evaluate-directional-1h` (16:42): evaluate only causally mature 1h coverage, abstention/missing reasons, calibration/reliability, Brier score, log loss, hit rate, ranking, and PRE/REGULAR/POST cohorts; the current stock day is still provisional.
5. `evaluate-directional-4h` (17:42): first require `equity_actionable_day_complete=true` and seal the complete PRE/REGULAR/POST stock day, then assess causally mature 4h quality by checkpoint session and isolate horizon-specific failure patterns.
