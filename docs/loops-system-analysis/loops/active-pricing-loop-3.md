...
> **Current deployment (2026-09-04):** the recurring Active Pricing supervisor
> is stopped. Its verified artifacts and pricing components remain inputs to the
> bounded overnight pipeline where compatible; recurrence details below are
...
| Canonical historical OPRA | One-time bootstrap plus Loop A-owned daily catch-up | verified `market-data/databento/opra/OPRA.PILLAR` definition, CBBO, and hourly-bar partitions | provider identity, exact request bounds, contract definitions, pretarget quotes, option OHLCV, raw/normalized checksums and event/availability clocks | Only receipt-verified nonempty partitions; definitions are joined point-in-time and quotes cannot follow their target; no provider request occurs in Pricing or its worker | Optional offline replay/model evidence; hourly bars and CBBO are production Strategy-profit dependencies; never fabricated when absent | **Confirmed.** `datafetching/orchestrate.py`, `datafetching/databento_opra_history.py`, `ml/option_pricing/opra_materialization.py`, `ml/strategy_profit_training_runtime.py` |
