...
3. **Confirmed:** construct horizon targets according to explicit exchange-calendar rules: next 60 or 180 eligible one-minute marks, next-session open/close, or the dynamic remaining-week aggregate and its contiguous eligible-session prefix. Both intraday decisions may use the latest completed bounded US extended-hours source bar from 04:00--20:00 Eastern. The `1h` start is the next PRE/REGULAR/POST segment open or complete local clock hour; `4h` uses 07:30/11:30/15:30/19:30 Eastern checkpoints. Eligible target minutes follow Schwab's 07:00--09:25, 09:30--16:00, and 16:05--20:00 segments, with early closes kept core-only. Sparse no-trade minutes use only a strictly prior native close, never a future value, and require collection coverage through the target end. Cost-adjusted return is positive only after one configured round trip. `ml/calendars.py`, `ml/horizons.py`, `ml/rolling_samples.py`
...
5. **Confirmed:** split target clusters chronologically into training, calibration, assessment and sealed lockbox; exclude target starts already used by compatible prospective LIVE forecasts. The bounded native-minute input uses 160/40/40/80 for `1h` and 128/32/32/64 for the lower-frequency, overnight-overlapping four-checkpoint `4h` route; both retain the 4:1:1:2 evidence proportions and train on every older eligible cluster. Daily/weekly routes retain 252/63/63/126. Fit or reuse an exact-compatible model and calibrator, never using the assessment or lockbox to train. `app/services/market_fetch_specs.py:96`, `ml/runtime_pipeline.py:464`, `ml/model_runtime.py:76`, `ml/model_runtime.py:141`, `ml/model_runtime.py:372`, `ml/model_runtime.py:458`
