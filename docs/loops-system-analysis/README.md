...
- **Confirmed:** `datafetching.options_history` is the one-time per-symbol OPRA bootstrap. Options Capture owns recurring catch-up for completed cursors; neither path creates an eighth supervisor. `datafetching/options_history.py`, `datafetching/options_runtime.py`
