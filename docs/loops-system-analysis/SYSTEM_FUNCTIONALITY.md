...
- Historical OPRA synchronization is a resumable Standard-subscription maintenance path. `ml.option_pricing_opra` discovers provider bounds, refuses nonzero-cost scope, preflights capacity, and publishes provider-native DBN plus normalized Parquet partitions. `datafetching.options_history` is the one-time per-parent bootstrap with frequency-specific initial windows (`ohlcv-1s` 5 days, `ohlcv-1m` 100 days, `ohlcv-1h` 2,000 days, `ohlcv-1d` and definitions 5,000 days, `cmbp-1` one month, remaining schemas six months). Options Capture only advances completed cursors with frequency-specific OHLCV overlaps of 1/2/5/10 days and a three-day overlap for other schemas; Pricing and Strategy consume only checksum-verified local partitions.
