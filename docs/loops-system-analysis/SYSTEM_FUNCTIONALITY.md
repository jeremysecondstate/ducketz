...
- Historical OPRA synchronization has three one-shot command boundaries. `datafetching.options_history` performs the normal included Standard-plan bootstrap independently for each parent symbol and schema: every `*-1s` schema uses 3 days, every `*-1m` schema 100 days, `ohlcv-1h` 1,825 days, `ohlcv-1d` 2,555 days, `definition` 13 calendar years, `cmbp-1` one day, and the other non-interval schemas one month. `datafetching.databento_cold_start` is the optional all-dataset bootstrap whose execution requires `--confirm-download`; its OPRA scopes use the same canonical storage and publish a v5 history-cursor handoff, while CME/US-equity outputs remain in an isolated archive. All datasets share the interval caps; definition windows are OPRA 13 calendar years, US Equities 8 calendar years, and CME 5,000 days, while dense `cmbp-1`, `mbp-10`, and `mbo` streams use one day. Both paths validate included scope and storage capacity. `ml.option_pricing_opra` is the separate full-universe or custom-scope administrative synchronizer. Options Capture then runs at most one catch-up per UTC date for valid cursors, using OHLCV overlaps of 1/2/5/10 days and three days for other schemas. Current writes are v5; legacy v4 is accepted only when its schema policy matches exactly. `datafetching/options_history.py`, `datafetching/databento_cold_start.py`, `datafetching/options_runtime.py:1018`, `datafetching/databento_opra_history.py`, `ml/option_pricing_opra.py`
...
| Active Pricing | 15 minutes, +1 | Waits up to 30 s in the startup command for exact Loop A readiness | **Confirmed.** `docs/datafetch-ml/current_start_command:94`, `docs/datafetch-ml/current_start_command:96` |
...
| Strategy | 15 minutes, +10 | Reads current Loop B and evidence; skip fingerprint is Loop B pointer + pricing mode + per-symbol Schwab snapshot heads | **Confirmed.** OPRA-only partition changes do not independently wake an otherwise unchanged cycle. `ml/strategy_runtime.py:120`, `ml/strategy_runtime.py:414` |
