...
- Historical OPRA synchronization has two command boundaries. `datafetching.options_history` performs the normal one-time bootstrap independently for each parent symbol and Standard schema: `ohlcv-1s` 5 days, `ohlcv-1m` 100 days, `ohlcv-1h` 2,000 days, `ohlcv-1d` and `definition` 5,000 days, `cmbp-1` one month, and the remaining schemas six months. Options Capture then runs at most one catch-up per UTC date only for completed v4 cursors, using OHLCV overlaps of 1/2/5/10 days and three days for other schemas. `ml.option_pricing_opra` is the separate full-universe or custom-scope administrative synchronizer. All three use the same entitlement discovery, capacity preflight, immutable partition, deduplication, and checksum verification contracts. `datafetching/options_history.py`, `datafetching/options_runtime.py`, `datafetching/databento_opra_history.py`, `ml/option_pricing_opra.py`
